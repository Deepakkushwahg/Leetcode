    ...
    public int minDifficulty(int[] jobDifficulty, int d) {
        ...
        for(int day=1;day<d;day++){
            for(int to=day;to<=len-d+day;to++){
                int currentDayDifficult = jobDifficulty[to];
                int result = Integer.MAX_VALUE;
                for(int j=to-1;j>=day-1;j--){
                    result = Math.min(result,(minDifficulty[day-1][j]+currentDayDifficult));
                    currentDayDifficult = Math.max(currentDayDifficult,jobDifficulty[j]);
                }
